...
function normalizeTask(task) {
  return {
    ...
    adaptationType: task.adaptationType || '',
...
function enforceWorkFocus(){
  ...
  if(active&&!['home','work'].includes(state.route))state.route='work';
...
function getDaySignals(date=todayISO()){
  const saved=(state.ui.daySignals||{})[date]||{};
  return {lowEnergy:Boolean(saved.lowEnergy||state.ui.lowEnergyDate===date),tight:Boolean(saved.tight||state.ui.tightDayDate===date),closed:Boolean(saved.closed||state.ui.dayClosedDate===date),absence:Boolean(saved.absence)};
}
function setDaySignal(date,key,value=true){
  state.ui.daySignals=state.ui.daySignals||{};const row={...(state.ui.daySignals[date]||{})};row[key]=Boolean(value);row.updatedAt=new Date().toISOString();state.ui.daySignals[date]=row;
  const keys=Object.keys(state.ui.daySignals).sort();while(keys.length>45){delete state.ui.daySignals[keys.shift()];}
  saveUI();
}
function isAnomalousDay(date=todayISO()){const s=getDaySignals(date);return s.lowEnergy||s.tight||s.absence;}
function getHistoricalCapacityFactor(){
  const rows=(state.learning.weeklySnapshots||[]).filter(x=>x.kind==='day-outcome'&&!x.anomalous&&Number(x.plannedMinutes)>0&&Number(x.completedMinutes)>=0).slice(-21);
  if(rows.length<4)return 1;const ratios=rows.map(x=>Math.max(.45,Math.min(1.15,Number(x.completedMinutes)/Number(x.plannedMinutes))));
  return Math.max(.78,Math.min(1.05,adeMedian(ratios)??1));
}
function recordAdaptationEvent(type,meta={}){state.learning.adaptationEvents=[...(state.learning.adaptationEvents||[]),{type,date:todayISO(),at:new Date().toISOString(),...meta}].slice(-120);saveLearning();}
function recordDayOutcomeSnapshot(date=todayISO()){
  const pending=tasksOn(date),done=state.tasks.filter(t=>t.completedAt&&toISO(new Date(t.completedAt))===date&&!t.deletedAt);
  const completedMinutes=done.reduce((s,t)=>s+Number(t.actualDuration||predictedDuration(t)||0),0),plannedMinutes=completedMinutes+pending.reduce((s,t)=>s+predictedDuration(t),0);
  const row={kind:'day-outcome',date,plannedMinutes,completedMinutes,anomalous:isAnomalousDay(date),signals:getDaySignals(date)};
  const list=(state.learning.weeklySnapshots||[]).filter(x=>!(x.kind==='day-outcome'&&x.date===date));list.push(row);state.learning.weeklySnapshots=list.slice(-42);saveLearning();
}
function getDayCapacity(date) {
  ...
  if(state.settings.intelligentMode)base*=getHistoricalCapacityFactor();
  if(getDaySignals(date).lowEnergy)base*=.82;
  if(state.ui.tightDayDate===date||getDaySignals(date).tight)base*=.65;
...
function getDurationRatio(category){
  const samples=(state.learning.durationSamples||[]).filter(s=>s.category===category&&Number(s.estimated)>0&&Number(s.actual)>0&&!s.anomalous).slice(-24);
...
function getDurationPrediction(task){
  ...
  const samples=(state.learning.durationSamples||[]).filter(x=>x.category===task?.category&&Number(x.actual)>0&&!x.anomalous).slice(-24);
...
function getDayLoad(date, extraMinutes=0, excludeTaskId=null) {
  ...
  const executed=state.tasks.filter(t=>t.id!==excludeTaskId&&!t.deletedAt&&t.completedAt&&toISO(new Date(t.completedAt))===date);
  const executedMinutes=executed.reduce((sum,t)=>sum+Number(t.actualDuration||predictedDuration(t)||0),0);
  const planned=dayTasks.reduce((sum,t)=>sum+Number(t.duration||0),0)+executedMinutes+extraMinutes;
  const mental=dayTasks.reduce((sum,t)=>sum+effectiveTaskLoadMinutes(t),0)+executedMinutes+extraMinutes;
  const capacity=getDayCapacity(date);
  return { planned, mental, capacity, percent: Math.round((mental/Math.max(1,capacity))*100), busy:getBusyMinutes(date), executedMinutes };
}
...
function getTemporalPriority(task,atDate=todayISO()){
  ...
  if(state.lowEnergyMode||getDaySignals(today).lowEnergy){if(task.energy==='low')score+=24;else if(task.energy==='high')score-=22;}
...
function getDayState(date=todayISO()){
  const tasks=tasksOn(date).filter(t=>!t.deletedAt&&!t.archivedAt),remaining=tasks.filter(t=>!t.completedAt),completed=state.tasks.filter(t=>!t.deletedAt&&t.completedAt&&toISO(new Date(t.completedAt))===date),blocked=remaining.filter(isTaskBlocked),overdue=remaining.filter(isOverdueDeadline);
...
function getBusyIntervals(date) {
  const intervals=[];
  state.externalEvents.filter(e=>e.date===date && Number(e.duration||0)>0).forEach(e=>{
    const start=externalStartMinutes(e); if(start!==null) intervals.push([Math.max(0,start-10),start+Number(e.duration||0)+10]);
  });
  tasksOn(date).filter(t=>t.scheduledTime).forEach(t=>{ const start=parseTimeMinutes(t.scheduledTime); if(start!==null) intervals.push([Math.max(0,start-5),start+predictedDuration(t)+5]); });
...
function getCurrentContext(){
  const active=state.ui.activeNowTaskId&&state.tasks.find(t=>t.id===state.ui.activeNowTaskId&&!t.completedAt);
  return active?contextFromTask(active):'';
}
function adeTaskDecisionScore(task,{available=60,currentContext=''}={}){
  let score=getTemporalPriority(task).score,dur=predictedDuration(task),reasons=[];
  if(task.deadline&&task.deadline<=todayISO()){score+=24;reasons.push(task.deadline<todayISO()?'está vencida':'vence hoy');}
  if(task.nonNegotiableDate===todayISO()){score+=28;reasons.push('no se debe mover hoy');}
  if(task.todayPriorityUntil===todayISO()){score+=20;reasons.push('está marcada como importante hoy');}
  if(currentContext&&contextFromTask(task)===currentContext){score+=12;reasons.push('mantiene el contexto actual');}
  if(dur<=available){score+=18;reasons.push('cabe en el hueco disponible');}
  else { const overflow=dur-available; score-=Math.min(32,overflow*.35); if(dur>=60&&overflow<=30){score+=16;reasons.push('protege una tarea de trabajo profundo');} }
  if(dur>=60&&getUsableGaps(todayISO()).some(g=>g.duration>=dur)){score+=15;reasons.push('hay un bloque continuo suficiente');}
  if(isTaskBlocked(task)){score-=300;reasons=['está bloqueada'];}
  return {task,score,duration:dur,reasons:[...new Set(reasons)].slice(0,3)};
}
function getADEDecision(){
  const day=getDayState(todayISO()),gaps=getUsableGaps(todayISO()),gap=gaps[0],available=gap?.duration||Math.max(15,day.capacity.remaining-day.remainingMinutes),currentContext=getCurrentContext();
  const pool=day.remainingTasks.filter(t=>!t.inbox&&!isTaskBlocked(t));
  const ranked=pool.map(t=>adeTaskDecisionScore(t,{available,currentContext})).sort((a,b)=>b.score-a.score||String(a.task.id).localeCompare(String(b.task.id)));
  const next=ranked[0]||null,moves=day.capacityRatio>100?getMoveSuggestions(todayISO(),85):[];
  let intervention=null;
  if(day.capacityRatio>100&&moves.length){const moved=moves.reduce((sum,x)=>sum+effectiveTaskLoadMinutes(x.task),0),after=Math.max(0,Math.round(((day.remainingMinutes-moved)/Math.max(1,day.capacity.remaining))*100));intervention={type:'space',severity:'danger',moves,after,reasons:[`el plan ocupa ${day.capacityRatio}% de la capacidad restante`,`basta con mover ${moves.length} ${moves.length===1?'tarea':'tareas'}`]};}
  else if(next) intervention={type:'next',severity:'calm',task:next.task,reasons:next.reasons};
  return {day,gaps,available,next,intervention,ranked};
}
function getNextBestAction() { return getADEDecision().next?.task||null; }
...
function getRecommendation() {
  const decision=getADEDecision(),today=decision.day;
  if(decision.intervention?.type==='space'){const i=decision.intervention;return {type:'space',className:'danger',text:`El plan ya no cabe · mover ${i.moves.length} → ${i.after}%`,action:'Ajustar',reason:i.reasons.join(' · ')};}
  if(state.ui.recoveryPendingDays>=3){const r=getRecoverySummary();if(r.important.length||r.movable.length||r.stale.length)return {type:'recovery',className:'warning',text:`Volver sin agobios · ${r.important.length} importantes`,action:'Ordenar',reason:'Prioriza lo importante antes de recuperar el resto.'};}
  const important=getImportantTasks(),overdue=important.filter(isOverdueDeadline);if(overdue.length)return {type:'overdue',className:'warning',text:`⏰ ${overdue.length} ${overdue.length===1?'tarea vencida':'tareas vencidas'}`,action:'Revisar',reason:'Las fechas vencidas tienen prioridad sobre sugerencias menores.'};
  const inbox=activeTasks().filter(t=>t.inbox);if(inbox.length>=3)return {type:'inbox',className:'',text:`${inbox.length} cosas por colocar`,action:'Ordenar',reason:'Reducir la entrada evita que queden tareas sin contexto.'};
  if(state.ui.activeNowTaskId&&state.tasks.some(t=>t.id===state.ui.activeNowTaskId&&!t.completedAt&&!t.archivedAt))return {type:'continue',className:'',text:`Continuar · ${state.tasks.find(t=>t.id===state.ui.activeNowTaskId)?.title||'tarea'}`,action:'Abrir',reason:'Continuar evita un cambio de contexto innecesario.'};
  const stale=getStaleTasks();if(stale.length)return {type:'stale',className:'',text:'Hay tareas antiguas por revisar',action:`Revisar ${Math.min(stale.length,4)}`,reason:'Conviene decidir si siguen teniendo sentido, no hacerlas por antigüedad.'};
  const tomorrow=getDayLoad(addDays(todayISO(),1));if(tomorrow.percent>110)return {type:'tomorrow',className:'warning',text:`Mañana viene al ${tomorrow.percent}%`,action:'Revisar',reason:'Una corrección pequeña hoy evita sobrecarga mañana.'};
  return null;
}
...
function getPlanningDayTasks(date){
  if(date<todayISO())return state.tasks.filter(t=>!t.archivedAt&&!t.deletedAt&&((t.completedAt&&toISO(new Date(t.completedAt))===date)||(!t.completedAt&&t.scheduledDate===date)));
  return state.tasks.filter(t=>!t.archivedAt&&!t.deletedAt&&!t.completedAt&&t.scheduledDate===date);
}
function getPlanningDayLoad(date){
  if(date>=todayISO())return getDayLoad(date);
  const tasks=getPlanningDayTasks(date),planned=tasks.reduce((sum,t)=>sum+(t.completedAt?(t.actualDuration??predictedDuration(t)):predictedDuration(t)),0);
  const capacity=getDayCapacity(date);
...
function createWeeklySnapshot(){const key=todayISO(),list=state.learning.weeklySnapshots||[];if(list.at(-1)?.date===key)return;list.push({date:key,planned:Array.from({length:7},(_,i)=>getDayLoad(addDays(todayISO(),i)).percent),active:activeTasks().length});state.learning.weeklySnapshots=list.slice(-26);saveLearning();}
...
function renderHome() {
  ...
  state.lowEnergyMode=getDaySignals(today).lowEnergy;els.lowEnergyBtn.classList.toggle('active',state.lowEnergyMode);
  const recommendation=getRecommendation();els.smartRecommendation.hidden=!recommendation;if(recommendation){els.smartRecommendation.className=`smart-recommendation ${recommendation.className||''}`;els.smartRecommendation.dataset.recommendation=recommendation.type;els.smartRecommendation.title=recommendation.reason||'';els.smartRecommendation.setAttribute('aria-label',`${recommendation.text}. ${recommendation.reason||''}`);els.smartRecommendation.innerHTML=`<span>${escapeHTML(recommendation.text)}${recommendation.reason?`<small class="recommendation-reason">${escapeHTML(recommendation.reason)}</small>`:''}</span><strong>${escapeHTML(recommendation.action)} →</strong>`;}
...
function renderDayAgenda() {
  ...
  const delta=Math.round((parseISODate(date)-parseISODate(todayISO()))/86400000);const sameWeek=Math.abs(delta)<=6;els.dayAgendaTitle.textContent=delta===0?'Hoy':delta===1?'Mañana':delta===2?'Pasado mañana':sameWeek?new Intl.DateTimeFormat('es-ES',{weekday:'long',day:'numeric'}).format(parseISODate(date)):formatDate(date,true);
...
function openWhatsNew(){
  showActionSheet(`${sheetHeader('Novedades','Organizador 6.0.1')}
    <div class="release-hero">
      ...
      <p>La 6.0 consolida el Adaptive Day Engine y la base técnica sin cambiar tu forma de trabajar: núcleo común, detección de plataforma centralizada y regresiones reproducibles.</p>
    </div>
...
function mutateWithUndo(label,mutator,{haptic=true,lockKey=''}={}){const key=lockKey||`mutate:${label}`;if(isActionLocked(key))return;const snapshot=deepClone(state.tasks);try{mutator();touchChangedTasks(snapshot);saveAll();render();pushUndo(label,snapshot);if(haptic)buzz(9);maybeCelebrateFocus();}finally{setTimeout(()=>releaseActionLock(key),ACTION_LOCK_MS);}}
function animatedMutation(id,type,label,mutator){if(isActionLocked(`task:${id}`))return;const rows=[...document.querySelectorAll(`[data-task-id="${CSS.escape(id)}"]`)];rows.forEach(r=>r.classList.add(type));const delay=reduceMotion()?0:145;setTimeout(()=>{releaseActionLock(`task:${id}`);mutateWithUndo(label,mutator,{lockKey:`mutate-task:${id}`});},delay);}
...
function completeTask(id,{actualMinutes=null,skipDurationPrompt=false}={}){
  ...
  if(actualMinutes==null&&task.duration==null&&!skipDurationPrompt){releaseActionLock(`complete:${id}`);openActualDurationPrompt(id);return;}
...
function confirmMakeSpace(){const moves=[...state.pendingSpaceSuggestions];if(!moves.length)return;mutateWithUndo('Día reorganizado',()=>moves.forEach(m=>{const t=state.tasks.find(x=>x.id===m.id);if(t){t.scheduledDate=m.date;t.adaptationType='systemReplanned';}}));state.pendingSpaceSuggestions=[];closeActionSheet();}
function applyDayClose(action){recordDayOutcomeSnapshot(todayISO());setDaySignal(todayISO(),'closed',true);state.ui.dayClosedDate=todayISO();recordAdaptationEvent('day-closed',{action});const pending=tasksOn(todayISO());mutateWithUndo('Cierre del día aplicado',()=>pending.forEach(task=>{if(task.nonNegotiableDate===todayISO())return;if(action==='tomorrow'){const tomorrow=addDays(todayISO(),1);task.scheduledDate=getDayLoad(tomorrow,effectiveTaskLoadMinutes(task),task.id).percent<=95?tomorrow:findNextGap(task,tomorrow);task.lastDecisionReason='capacity';task.adaptationType='systemReplanned';}if(action==='gaps'){task.scheduledDate=findNextGap(task);task.lastDecisionReason='capacity';task.adaptationType='systemReplanned';}if(action==='archive')task.archivedAt=new Date().toISOString();}));closeActionSheet();}
...
function toggleLowEnergy(){state.lowEnergyMode=!getDaySignals(todayISO()).lowEnergy;state.ui.lowEnergyDate=state.lowEnergyMode?todayISO():'';setDaySignal(todayISO(),'lowEnergy',state.lowEnergyMode);recordAdaptationEvent(state.lowEnergyMode?'low-energy-on':'low-energy-off');renderHome();renderContextIsland();if(state.lowEnergyMode){document.body.classList.add('home-results-open');state.quickMode='low-energy';const tasks=activeTasks().filter(t=>t.energy==='low'||(t.energy==='normal'&&predictedDuration(t)<=25)).sort((a,b)=>taskScore(b)-taskScore(a)).slice(0,8);els.smartResultsTitle.textContent='Poco esfuerzo, buen avance';renderSmartList(tasks);els.smartResults.hidden=false;}else{els.smartResults.hidden=true;state.quickMode=null;document.body.classList.remove('home-results-open');}}
...
function updateLastOpened(){const previous=state.ui.lastOpenedDate;const today=todayISO();if(previous&&previous<today){const days=Math.max(0,dayDistance(today)-dayDistance(previous));state.ui.recoveryPendingDays=days>=3?days:0;if(days>=3)setDaySignal(today,'absence',true);}state.ui.lastOpenedDate=today;state.ui.lastOpenedAt=new Date().toISOString();saveUI();}
...
els.taskForm.addEventListener('submit',event=>{
  ...
  saveAll();clearTaskDraft();document.activeElement?.blur?.();cancelActiveGesture();releaseSuppressedClicks();closeSheet(els.taskSheet,{restoreFocus:false});render();pushUndo(existing?'Tarea actualizada':'Tarea creada',snapshot);
...
function updateKeyboardLockUI(){
  if(!els.keyboardLockBtn)return;const show=isWindowsPWA();els.keyboardLockBtn.hidden=!show;const locked=Boolean(state.ui.keyboardShortcutsLocked);els.keyboardLockBtn.classList.toggle('active',locked);els.keyboardLockBtn.setAttribute('aria-pressed',String(locked));els.keyboardLockBtn.title=locked?'Atajos bloqueados':'Bloquear atajos';els.keyboardLockBtn.setAttribute('aria-label',locked?'Activar atajos de teclado':'Bloquear atajos de teclado');
}
if(els.keyboardLockBtn)els.keyboardLockBtn.addEventListener('click',()=>{state.ui.keyboardShortcutsLocked=!state.ui.keyboardShortcutsLocked;saveUI();updateKeyboardLockUI();toast(state.ui.keyboardShortcutsLocked?'Atajos de teclado bloqueados.':'Atajos de teclado activados.');});
updateKeyboardLockUI();
/* Zoom intentionally locked for this installed-app experience. */
...
function syncIOSVisualViewport(){
  if(!isIOS()||!window.visualViewport)return;document.documentElement.style.setProperty('--visual-vh',`${window.visualViewport.height}px`);const open=topOpenOverlay();if(open)open.style.height=`${window.visualViewport.height}px`;
}
if(window.visualViewport){visualViewport.addEventListener('resize',syncIOSVisualViewport);visualViewport.addEventListener('scroll',syncIOSVisualViewport);}syncIOSVisualViewport();
...
function refreshWorkFocusFromClock(){
  if(!isWindowsPWA())return;
  const before=state.workFocusWasActive,active=isWorkFocusActive();
  if(active!==before){enforceWorkFocus();render();}
}
setInterval(refreshWorkFocusFromClock,15000);
window.addEventListener('focus',refreshWorkFocusFromClock);
...
initCompactSettings();auditCapabilities();cleanupOldTrash();createWeeklySnapshot();updateLastOpened();ensureDailyFocus();render();checkReminders();updateSyncUI();processShareTarget();processLaunchAction();setTimeout(()=>{if(state.security.enabled||state.security.biometricCredentialId)showLocalLock();},250);initCloudSync().catch(error=>{console.error('Firebase sync:',error);setSyncStatus('error','La sincronización con Firebase no pudo iniciarse.');});

/* iOS PWA: when the keyboard changes the visual viewport, keep the edited field reachable. */
document.addEventListener('focusin',event=>{
  if(!isIOS()||!event.target.closest?.('.modal-layer.is-open input,.modal-layer.is-open textarea,.modal-layer.is-open select'))return;
  setTimeout(()=>event.target.scrollIntoView?.({block:'center',behavior:reduceMotion()?'auto':'smooth'}),180);
});
