# Organizador personal inteligente · v3.1.3

Revisión de estabilidad centrada en los dos fallos observados en iPhone/Windows: interacción con las tareas y menús flotantes, y actualización/visualización del icono de la aplicación.

## Cambios 3.1.3

### Interacción de tareas y menús flotantes
- Se elimina el uso de `<dialog>.showModal()` para los bottom sheets. Los paneles pasan a ser capas propias (`div` + overlay fijo), evitando los estados bloqueados que Safari/iOS puede dejar al mezclar `dialog`, pulsación larga y `pointercancel`.
- Un toque normal sobre el cuerpo de una tarea abre sus acciones directamente en `pointerup`; ya no depende del `click` sintético de Safari.
- El botón `…` de cada tarea tiene un manejador táctil directo e independiente.
- La supresión de clics posterior a swipe/long-press queda limitada exclusivamente a la fila de la tarea; nunca bloquea navegación, botones ni controles de un panel.
- `X`, Cancelar, Cerrar, Completar, Editar, Aplazar, Dividir, Prioridad/Energía, Mover, Archivar y Eliminar se ejecutan por `pointerup`, con `click` como respaldo para teclado/VoiceOver.
- `pointercancel`, `touchcancel`, pérdida de foco y cambio de visibilidad limpian cualquier gesto residual.
- Escape sigue cerrando los paneles en escritorio.

### Icono/PWA
- Nuevo juego de iconos versionados `v313` para evitar reutilizar URLs antiguas cacheadas por iOS/Windows.
- Apple Touch Icon 180×180, iconos PWA 192×192 y 512×512, variantes `maskable`, favicon PNG 32/64 y `favicon-v313.ico` multiresolución (con `favicon.ico` como fallback) en la raíz.
- El icono también aparece dentro de la interfaz de escritorio en la marca lateral.
- `manifest.webmanifest`, CSS, JS y registro del Service Worker llevan versión `3.1.3`.
- El Service Worker usa red primero para recursos de la app y `cache: no-store`, con caché como respaldo offline. Esto evita que CSS, JavaScript o iconos permanezcan anclados a una revisión antigua.

### Actualización desde una instalación anterior
- En iPhone, un acceso ya existente puede conservar su icono antiguo porque iOS lo guarda fuera de la caché normal de la página. Tras desplegar 3.1.3, elimina una vez el acceso de la pantalla de inicio y vuelve a añadirlo desde Safari.
- En Windows, si una PWA ya estaba instalada con otro icono y el sistema no lo refresca inmediatamente, desinstálala y vuelve a instalarla una vez. La pestaña web utiliza además `favicon.ico`/PNG nuevos.

## Validación previa al empaquetado
- Sintaxis de `app.js` y `sw.js` validada.
- Manifest JSON validado y tamaños reales de todos los iconos comprobados.
- Comprobación de IDs/estructura de los paneles y ausencia de `<dialog>` nativo.
- Regresión estática de toque de tarea, botón `…`, cierre de overlays y acciones críticas.
- Verificación de que todos los recursos del shell del Service Worker existen.

> No se ha realizado una prueba física en Safari/iPhone dentro de este entorno; la corrección se ha validado por estructura, eventos, sintaxis y recursos antes de empaquetar.
